function handlePlot() {
    const r1 = document.getElementById("bitstream-input").value;
    const ini = document.getElementById("initial-input").value;
    const final = document.getElementById("final-input").value;
    const alg = document.getElementById("algorithm").value;
    const side = document.getElementById("direction").value;
    console.log(ini);
    getBitStreamAndPlot(null, r1, ini, final, alg, side);
}

var pre, v1, v2, v3, v4, v5, v6, v7, v8, dir;
function getBitStreamAndPlot(_event, r1, ini, final, alg, side) {
    var inp = [], r2 = r1.split(" "), r3;
    for (a1 = 0; a1 < r2.length; ++a1) {
        if (r2[a1] == "") { continue; }
        r3 = parseInt(r2[a1]);
        inp.push(r3);
        if ((r3 > parseInt(final))) {
            console.log(r3);
            console.log(ini);
            console.log(final);
            alert("Invalid Input: Final cylinder has to be greater than all other cylinders");
            return;
        } else if ((parseInt(ini) > parseInt(final))) {
            console.log(r3);
            console.log(ini);
            console.log(final);
            alert("Invalid Input: Final cylinder has to be greater than the initial cylinder");
            return;
        } else if ((r3 > parseInt(final)) && (parseInt(ini) > parseInt(final))) {
            console.log(r3);
            console.log(ini);
            console.log(final);
            alert("Invalid Input: Final cylinder has to be greater than initial cylinder and all the other cylinders");
            return;
        }
